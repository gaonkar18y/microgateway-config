...
module.exports.validate = function validate(config) {
  ...
  if (config.edgemicro.hasOwnProperty('headers_timeout')) {
    let headers_timeout_message = 'invalid value for config.edgemicro.headers_timeout: ' + config.edgemicro.headers_timeout;
    assert(typeof config.edgemicro.headers_timeout === 'number', headers_timeout_message);
    assert(+config.edgemicro.headers_timeout > 0, headers_timeout_message);
  }
  ...
  if (config.quotas) {
    ...
    Object.keys(config.quotas).forEach(key => {
      assert( key === 'failOpen' || key === 'useDebugMpId' || key === 'useRedis' || key === 'bufferSize', 'invalid value in config.quotas: ' + key +
              ', valid values are failOpen, useDebugMpId, useRedis, bufferSize');
      if (  key === 'failOpen') {
