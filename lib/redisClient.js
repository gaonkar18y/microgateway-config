...
const redisClientConnection = function(config, cb) {
    ...
    const useRedisCluster = config.useRedisCluster || false
    const redisCluster = config.redisCluster || [ { host, port } ]
   ...
   if ( useRedisCluster === true ) {
    debug('using redis cluster');
    this.redisClient = new Redis.Cluster(redisCluster);
   } else {
    debug('no redis cluster');
    this.redisClient = new Redis({ port, host, password:  options['auth_pass'], db })
   }
