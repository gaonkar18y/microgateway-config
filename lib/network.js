 ...
 * @param config the config for redis host and port.
 * 
 */
 ...
 * @param config the org-env-config.yaml config
 * @param options the options passed in edgemicro start
 */
Loader.prototype.getConfigFromRedis = function (config, options, cb) {
    this.connectRedisDb(config.edgemicro);
    this.redisClient.get(options.org+'_'+options.env, function(err, reply){
        if (!err) {
