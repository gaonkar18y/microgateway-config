    ...
    async.parallel([
        function(cb) {
            ...
            if (process.env.EDGEMICRO_LOCAL_PROXY === "1") {
                ...
                _loadStatus('config', config.edge_config.bootstrap, null, response, proxyInfo, cb, config);
            } else { //retrieve info from edge
                request.get(opts, function(err, response, body) {
                    _loadStatus('config', config.edge_config.bootstrap, err, response, body, cb, config);
                });
            }
        },
        function(cb) {
            ...
            request.get(opts, function(err, response, body) {
                _loadStatus('products', config.edge_config.products, err, response, body, cb, config);
            });
        },
        function(cb) {
            var opts = _.clone(options);
 ...
 * @private
 */
const _loadStatus = function(message, url, err, response, body, cb, config) {
    const failed = err || (response && response.statusCode !== 200);
    let gettingBackupFromRedis = false;
    if (url) {
        ...
        if ( failed ) {
           ...
           if ( config.edgemicro.useRemoteCache === true ) {
            const loader = new Loader();
            gettingBackupFromRedis = true;
            loader.getConfigFromRedis(config, message, function(err, body){
                if (body) {
                    writeConsoleLog('log',{component: CONSOLE_LOG_TAG_COMP},'using remote cached configuration from %s for key: %s', config.edgemicro.redisHost, message);
                }
                cb(err, body);
            });
           }
            ...
            if ( config.edgemicro.cacheSynchornizer === true ) {
                const loader = new Loader();
                writeConsoleLog('log',{component: CONSOLE_LOG_TAG_COMP},'Saving cache to remote host: %s for key: %s', config.edgemicro.redisHost, message);
                loader.uploadToRedis(config, body, message);
            }
            writeConsoleLog('info',{component: CONSOLE_LOG_TAG_COMP},message, 'download from', url, 'returned',
                            (response ? (response.statusCode + ' ' + response.statusMessage) : '', err ? err : ''));
        }
    }
    if ( !gettingBackupFromRedis ){
        if (err) {
            cb(err, body);
 ...
 * 
 */
Loader.prototype.connectRedisDb = function (config) {
    if ( !this.client ) {
        var host = config.redisHost || '127.0.0.1';
        var port = config.redisPort || 6379;
        var db = config.redisDb || 0;
 ...
 */
Loader.prototype.uploadToRedis = function (config, data, suffixKey) {
    this.connectRedisDb(config.edgemicro);
    let key = globalOptions.org+'_'+globalOptions.env;
 ...
 * @param options the options passed in edgemicro start
 */
Loader.prototype.getConfigFromRedis = function (config, suffixKey, cb) {
    this.connectRedisDb(config.edgemicro);
    let key = globalOptions.org+'_'+globalOptions.env;
    if ( suffixKey && typeof suffixKey === 'string' ){
        key+='_'+suffixKey;
